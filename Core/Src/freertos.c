...
void MotorControl(void *argument)
{
  ...
  for (;;)
  {
    ...
    test_msgss[0] = MOTOR_REAL_INFO[1].RPM;
    test_msgss[1] = motorleft_r.setpoint;
    test_msgss[2] = shoot_down_left.p_out;
    test_msgss[3] = SHOOT_MOTOR_INFO[0].real_target_current;
...
void RobotMove(void *argument)
{
  ...
  //	const portTickType move_xFrequency = pdMS_TO_TICKS(10); // ÑÓÊ±10ms
  /* Infinite loop */
  for (;;)
  {
    MotorCtrl();
    ...
    if (xQueueReceive(uartQueue, &msg, portMAX_DELAY) == pdPASS)
    {
      ...
      case UART_ID_USART1:
        ...
        if (mat_id == 0x01)
        {
          ...
          // motorleft_r.setpoint = rx_frame_mat.data.msg_get[0];
          // PID_SetParameters(&motorleft_r, rx_frame_mat.data.msg_get[1], rx_frame_mat.data.msg_get[2], rx_frame_mat.data.msg_get[3]);
